...
//    assign error_counter_o = (rd_data_o !== correct_count_o);
   ...
   dut
     (.clk_i(clk_i)
     ,.next_i(up_i)
     ,.arrows_o(count_o[7:4])
     ,.timing_o(count_o[3:0]));
...
`endif
   ...
   always @(negedge clk_i) begin
      ...
      if(reset_done & !reset_i & error_counter_o) begin
	    $display("\033[0;31mError!\033[0m: data_o should be %b, got %b", correct_count_o, count_o);
        error_counter_o = 1'b1;
      end else if (reset_i & error_counter_o) begin
	    $display("\033[0;31mError!\033[0m: data_o should be %b during reset, got %b", correct_count_o, count_o);
        error_counter_o = 1'b1;
      end
   end
